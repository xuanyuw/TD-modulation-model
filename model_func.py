...
def trial(par, train=True):
    ...
    if train and  par['save_train_out']:
        train_file = tables.open_file(join(par['save_dir'], 'train_output_lr%f_rep%d.h5' % (
    ...
    if not train and par['save_test_out']:
        test_file = tables.open_file(join(par['save_dir'], 'test_output_lr%f_rep%d.h5' % (
            par['learning_rate'], par['rep'])), mode='w', title='Test output')
        for n in range(len(all_idx)):
    ...
    if train: # plot train accuracy and loss
        plot_acc(model_performance['total_accuracy'], model_performance['loss'], par['save_dir'], par['learning_rate'], par['rep'])
...
def plot_acc(all_arr, loss, f_dir, lr, rep, h_arr = None, m_arr = None, l_arr = None, z_arr = None):
    ...
    ax2.set_title('Loss')
    plt.savefig(join(f_dir, 'TrainAcc_lr%f_rep%d.png' %
                (lr, rep)), format='png')
    # plt.show()

...
data_dir = "F:\Github\TD-modulation-model\dPCA_allTrial_plots\sepSac"
...
def main():
    ...
    for mType in model_types:
        ...
        for rep in tqdm(range(total_rep)):
            if mType == "cutSpec" and rep == 8:
                dir_arr.append(np.nan)
                targArr_arr.append(np.nan)
                condInd_arr.append(np.nan)
                inter_arr.append(np.nan)
                continue
            data = loadmat(
                os.path.join(
        ...
        explVar_dict_inter[mType] = inter_arr

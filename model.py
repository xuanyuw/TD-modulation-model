...
class Model(bp.layers.Module):
    ...
    def update(self, input, **kwargs):
        ...
        self.h.value = self.h * bm.relu((1-self.alpha)) + self.alpha * (input @ self.w_in + bm.relu(
            h_post) @ self.w_rnn + self.b_rnn) + bm.random.normal(0, self.noise_rnn, self.h.shape)

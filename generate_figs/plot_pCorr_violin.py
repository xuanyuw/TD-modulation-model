...
def main():
        ...
        if 'shuf' not in f_dir:
            ...
            for rep in all_rep:
                for shuf in range(total_shuf):
                    extract_pCorr_data(rep, m1_rng, True, motion_selective[rep, m1_rng], shuf)
                    extract_pCorr_data(rep, m2_rng, False, motion_selective[rep, m2_rng], shuf)
                    pbar.update(1)
            pbar.close()
    ...
    if reload_data:
        ...
        for r in all_rep:
            pCorr_results = loadmat(os.path.join(f_dir, 'pCorr_data', 'single_pCorr_result_rep%d.mat'%(r)))
    ...
    pairs = [(('stim', 'ipsi'), ('stim', 'contra')), (('choice', 'ipsi'), ('choice', 'contra'))]

    f =  open(os.path.join(plt_dir, "stat_test.txt"), 'w') 
    sys.stdout = f

    annot = Annotator(ax, pairs, data=df, x='r_type', y='mean_pCorr', hue='sac_dir', order=['stim', 'choice'])
...
def extract_pCorr_data(rep, motion_rng, m1, m_sel, shuf=None, coh_code = {'H':4, 'M':2, 'L':1, 'Z':0}, pref_sign = 1, non_sign = -1):
    if shuf is None:
    ...
    if m1:
        if shuf is None:
            savemat(os.path.join(f_dir, 'pCorr_data', 'single_pCorr_data_rep%d_m1.mat'%rep), data_dict)
        else:
            savemat(os.path.join(f_dir, 'pCorr_data', 'single_pCorr_data_rep%d_shuf%d_m1.mat'%(rep, shuf)), data_dict)
    else:
        if shuf is None:
            savemat(os.path.join(f_dir, 'pCorr_data', 'single_pCorr_data_rep%d_m2.mat'%rep), data_dict)
        else:
            savemat(os.path.join(f_dir, 'pCorr_data', 'single_pCorr_data_rep%d_shuf%d_m2.mat'%(rep, shuf)), data_dict)

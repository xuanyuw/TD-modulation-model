...
def main():
    ...
    # for i in motion_rng:
    #     plot_avgAct_combined(h, n, i, True, mode='motion'f_dir = f_dir,)
    for i in target_rng:
        if i in np.concatenate((np.arange(40, 80), np.arange(170, 180))):
            plot_avgAct_combined(h, n, i, True, mode="target", f_dir=f_dir, m1=True)
        else:
            plot_avgAct_combined(h, n, i, True, mode="target", f_dir=f_dir, m1=False)

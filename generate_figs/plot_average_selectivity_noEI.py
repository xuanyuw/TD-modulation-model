...
def main(lr, total_rep):
    if rerun_calculation:
        dir_sel_norm, sac_sel_pvnp_norm, sac_sel_lvr_norm = load_all_activities(lr, total_rep, True, plot_sel, plot_shuf)
    ...
    if load_shuf:
        ...
        for rep in range(total_rep):
            for shuf in range(total_shuf):
                motion_dir_sel, sac_sel_pvnp, sac_sel_lvr = load_single_activity(rep, lr, normalize, plot_sel, m_idx, m1_id, m2_id, shuf)
                if rep==0:
        ...
        for rep in range(total_rep):
            motion_dir_sel, sac_sel_pvnp, sac_sel_lvr = load_single_activity(rep, lr, normalize, plot_sel, m_idx, m1_id, m2_id)
            if rep==0:
                all_motion_dir_sel = motion_dir_sel
                all_sac_sel_pvnp = sac_sel_pvnp
                all_sac_sel_lvr = sac_sel_lvr

...
def plot_pCorr_data(rep):
    if isinstance(rep, int):
        pCorr_results = loadmat(os.path.join(f_dir, 'pCorr_data', 'pCorr_result_rep%d.mat'%(rep)))
        ...
        for r in rep:
            ...
            contra_pCorr_stim[:,r] = np.mean(pCorr_results['contra_pCorr_stim'], axis=1)
            ipsi_pCorr_choice[:,r] = np.mean(pCorr_results['ipsi_pCorr_choice'], axis=1)
            contra_pCorr_choice[:,r] = np.mean(pCorr_results['contra_pCorr_choice'], axis=1)
...
def create_plot(data_dict):
    ...
    ax1.fill_between(np.arange(h_len), ipsi_choice_mean-ipsi_choice_ste, ipsi_choice_mean+ipsi_choice_ste, color='#808080', alpha=0.5)
    ax1.fill_between(np.arange(h_len), contra_choice_mean-contra_choice_ste, contra_choice_mean+contra_choice_ste, color='#808080', alpha=0.5)
    ...
    ax2.fill_between(np.arange(len(ipsi_stim_mean)), ipsi_stim_mean-ipsi_stim_ste, ipsi_stim_mean+ipsi_stim_ste, color='#808080', alpha=0.5)
    ax2.fill_between(np.arange(len(contra_stim_mean)), contra_stim_mean-contra_stim_ste, contra_stim_mean+contra_stim_ste, color='#808080', alpha=0.5)
...
def extract_pCorr_data(rep, motion_rng, m1, m_sel, coh_code = {'H':4, 'M':2, 'L':1, 'Z':0}, pref_sign = 1, non_sign = -1):
    ...
    if m1:
        savemat(os.path.join(f_dir, 'pCorr_data', 'pCorr_data_rep%d_m1.mat'%rep), data_dict)
    else:
        savemat(os.path.join(f_dir, 'pCorr_data', 'pCorr_data_rep%d_m2.mat'%rep), data_dict)

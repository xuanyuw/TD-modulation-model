...
def main(lr, total_rep):
    ...
    plot_total_dir_selectivity(
        dir_sel_norm,
        "%s_Motion_dir_sel_norm_avg_combined" % model_type,
        True,
        plot_sel=plot_sel,
    )
    # plot_total_dir_selectivity(dir_sel_orig, "%s_Motion_dir_sel_raw_avg_combined"%model_type, True, plot_sel=plot_sel)

    plot_total_sac_selectivity_pvnp(
        sac_sel_pvnp_norm,
        "%s_Target_sac_sel_pvnp_norm_avg_combined" % model_type,
        True,
        plot_sel=plot_sel,
    )
    ...
    plot_total_sac_selectivity_lvr(
        sac_sel_lvr_norm,
        "%s_Target_sac_sel_lvr_norm_avg_combined" % model_type,
        True,
        plot_sel=plot_sel,
    )
...
def load_new_line_dict(old_dict):
    ...
    for coh in coh_list:
        for l in line_list:
            new_dict["%s_%s" % (coh, l)] = np.mean(
                np.vstack(
                    [old_dict["%s_%s_ax1" % (coh, l)], old_dict["%s_%s_ax2" % (coh, l)]]
                ),
                axis=0,
            )
    return new_dict
...
def plot_coh_popu_act(
    ...
    plt.vlines(
        [target_st_time - 20, stim_st_time - 20],
        yl[0],
        yl[1],
        color="k",
        linewidth=1,

...
from utils import find_sac_idx, combine_idx, load_test_data, min_max_normalize, find_coh_idx, get_pref_idx
...
def plot_violin(df, roc_type):
    ...
    handles = []
    sns.violinplot(x = 'coh', y = 'roc', hue='model', data = df.astype({'roc':float}), inner='points', ax=ax,  palette=colors, hue_order=['Full model', 'No feedback', 'Shuffled feedback'], order=['H', 'M', 'L', 'Z'])
    ...
    ax.legend(handles=[tuple(handles[::3]), tuple(handles[1::3]), tuple(handles[2::3])], labels=df["model"].astype('category').cat.categories.to_list(),
            handlelength=4, handler_map={tuple: HandlerTuple(ndivide=None, pad=0)}, loc='best', frameon=False)
    ...
    plt.savefig(os.path.join(plt_dir, "%s_roc_net_comp_comb_sac.pdf"%roc_type), format='pdf')
    plt.savefig(os.path.join(plt_dir, "%s_roc_net_comp_comb_sac.eps"%roc_type), format='eps')
    plt.savefig(os.path.join(plt_dir, "%s_roc_net_comp_comb_sac.png"%roc_type), format='png')
...
def plot_violin_sep_sac(df, model_type):
    ...
    colors = ['#FF0000', '#00FF00', '#0000FF', '#424242']
    handles = []
    sns.violinplot(x = 'coh', y = 'roc', hue = 'sac', data = df.astype({'roc':float}), inner='quartile', ax=ax,  palette=['.2', '.5'], hue_order=['ipsi', 'contra'], split=True, order=['H', 'M', 'L', 'Z'])
    ...
    ax.legend(handles=[tuple(handles[1::2]), tuple(handles[::2])], labels=df["sac"].astype('category').cat.categories.to_list(),
    ...
    annot = Annotator(ax, pairs, data=df, x='coh', y='roc', hue='sac', order=['H', 'M', 'L', 'Z'])
    annot.configure(test='t-test_ind', text_format='star', loc='outside')
    annot.apply_and_annotate()
    ...
    plt.tight_layout()
    if save_plot:
        plt.savefig(os.path.join(plt_dir, "dir_sel_net_comp_%s.pdf"%model_type), format='pdf')
        plt.savefig(os.path.join(plt_dir, "dir_sel_net_comp_%s.eps"%model_type), format='eps')
        plt.savefig(os.path.join(plt_dir, "dir_sel_net_comp_%s.png"%model_type), format='png')
    
    df_mean = df[['sac', 'coh', 'roc']].groupby(['sac', 'coh']).mean()
    df_mean.to_csv(os.path.join(plt_dir, "all_model_sep_sac_roc_mean_%s.csv"%model_type))

def plot_sac_roc_diff(df):
    df = df[(df['model']=='Full model')| (df['model']=='Shuffled feedback')]
    temp_idx = df['sac']=='contra'
    temp_df = df[temp_idx]
    roc_diff = df[df['sac']=='contra']['roc'].reset_index(drop=True)-df[df['sac']=='ipsi']['roc'].reset_index(drop=True)
    new_df = pd.DataFrame({'rep':temp_df['rep'].reset_index(drop=True), 'coh':temp_df['coh'].reset_index(drop=True), 'model':temp_df['model'].reset_index(drop=True), 'roc_diff':roc_diff})

    fig, ax = plt.subplots()
    colors = ['#FF0000', '#00FF00', '#0000FF', '#424242']
    sns.stripplot(x = 'coh', y = 'roc_diff', hue='model', data = new_df, ax=ax, 
                palette=['.2', '.5'], dodge=True,
    ...
    plt.tight_layout()
    if save_plot:
        plt.savefig(os.path.join(plt_dir, 'dir_sel_diff_full_vs_shuf.pdf'), format='pdf')
        plt.savefig(os.path.join(plt_dir, 'dir_sel_diff_full_vs_shuf.eps'), format='eps')
        plt.savefig(os.path.join(plt_dir, 'dir_sel_diff_full_vs_shuf.png'), format='png')



def load_single_file_ROC(f_dir, rep, motion_rng, m1_rng, motion_idx, target_idx, motion_selective, saccade_selective, shuf=None):
    if shuf is None:
    ...
    for coh in ['H', 'M', 'L', 'Z']:
        ipsi_roc, contra_roc = calc_sac_sep_ROC(h[50:, :, motion_rng],  n, m1_rng, coh_dict[coh], pref_dir[:, motion_rng])

        dir_roc = calc_ROC(h[50:, :, m_sel_rng], n, coh_dict[coh], pref_dir[:, m_sel_rng])
        sac_roc = calc_ROC(h[60:, :, s_sel_rng], n, coh_dict[coh], pref_sac[:, s_sel_rng]) # use 100ms before and after mean rt to calculate saccade sel
        ...
        temp_sep_sac_df = pd.concat((temp_sep_sac_df, pd.DataFrame({'rep': rep_arr, 'shuf': shuf_arr, 'coh': coh_arr, 'sac':sac_arr, 'roc': sep_sac_arr})))
        temp_comb_sac_df = pd.concat((temp_comb_sac_df, pd.DataFrame({'rep': rep_arr, 'shuf': shuf_arr, 'coh': coh_arr, 'type':type_arr, 'roc': comb_sac_arr})))
...
def calc_all_ROC():
    ...
    for f_dir in f_dirs:
        if 'shuf' not in f_dir:
            ...
            comb_sac_roc_df = pd.DataFrame(columns=['rep', 'shuf', 'coh', 'type', 'roc'])
            for rep in all_rep:
                temp_sep_sac_df, temp_comb_sac_df = load_single_file_ROC(f_dir, rep, motion_rng, m1_rng, motion_idx, target_idx, motion_selective, saccade_selective)
            ...
            for rep in all_rep:
                ...
                for shuf in range(total_shuf):
                    temp_sep_sac_df, temp_comb_sac_df = load_single_file_ROC(f_dir, rep, motion_rng, m1_rng, motion_idx, target_idx, motion_selective, saccade_selective, shuf)
                    temp_sep_sac_df['model'] = ['Shuffled feedback']*len(temp_sep_sac_df.index)
                    temp_comb_sac_df['model'] = ['Shuffled feedback']*len(temp_comb_sac_df.index)
                ...
                sep_sac_roc_df = sep_sac_roc_df.explode('roc').reset_index()
                comb_sac_roc_df = comb_sac_roc_df.explode('roc').reset_index()
                sep_sac_roc_df.to_csv(os.path.join(data_dir, 'shufFeedback_all_nets_ROC_sep_sac_rep%d.csv'%rep))
                comb_sac_roc_df.to_csv(os.path.join(data_dir, 'shufFeedback_all_nets_ROC_comb_sac_rep%d.csv'%rep))
            pbar.close()
...
def load_roc():
    if rerun_calc:
        ...
        for i in mn:
            if 'shuf' in i:
                for rep in all_rep:
                    temp_sep_sac = pd.read_csv(os.path.join(data_dir, '%s_all_nets_ROC_sep_sac_rep%d.csv'%(i, rep)))[['rep', 'coh', 'sac', 'model', 'roc']].groupby(['rep', 'coh', 'sac', 'model']).mean().reset_index()
                    temp_comb_sac = pd.read_csv(os.path.join(data_dir, '%s_all_nets_ROC_comb_sac_rep%d.csv'%(i, rep)))[['rep', 'coh', 'type', 'model', 'roc']].groupby(['rep', 'coh', 'type', 'model']).mean().reset_index()
                    sep_sac_df = pd.concat([sep_sac_df, temp_sep_sac])
                    comb_sac_df = pd.concat([comb_sac_df, temp_comb_sac])
            else:
                temp_sep_sac = pd.read_csv(os.path.join(data_dir, '%s_all_nets_ROC_sep_sac.csv'%i))[['rep', 'coh', 'sac', 'model', 'roc']].groupby(['rep', 'coh', 'sac', 'model']).mean().reset_index()
                temp_comb_sac = pd.read_csv(os.path.join(data_dir, '%s_all_nets_ROC_comb_sac.csv'%i))[['rep', 'coh', 'type', 'model', 'roc']].groupby(['rep', 'coh', 'type', 'model']).mean().reset_index()
...
def calc_sac_sep_ROC(h, n, m1_rng, coh_idx, pref_idx):
    ...
    # tic = perf_counter()
    return_li = Parallel(n_jobs=n_jobs)(delayed(cell_roc_sep_sac)(i) for i in range(h.shape[2]))
    # toc=perf_counter()

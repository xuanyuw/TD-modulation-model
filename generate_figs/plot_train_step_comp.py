...
plt_dir = os.path.join('generate_figs', 'rmv_fb_plots', 'train_step_comp')
...
for i, f_dir in enumerate(f_dirs):
    ...
    for rep in range(total_rep):
        train_table = tables.open_file(os.path.join("crossOutput_noInterneuron_noMTConn_removeFB_model", "train_output_lr0.020000_rep%d.h5"%rep), mode='r')
...
fig, ax = plt.subplots()
sns.stripplot(x="model", y="max_iter", data=df, dodge=True, palette=colors, alpha=.8)
# plot the mean line

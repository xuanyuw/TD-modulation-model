...
def plot_below_thresh_cnt(below_thresh_count):
    below_thresh_count.columns = below_thresh_count.columns.droplevel(1) #drop lambda multi-index
    _, ax = plt.subplots()
    colors = ['#FF0000', '#00FF00', '#0000FF', '#424242']
    handles = []
    # sns.violinplot(x = 'coh', y = 'rt', hue = 'model', data = below_thresh_count, inner='points', ax=ax,  palette=['.2', '.5'], hue_order=['Full model', 'No feedback', 'Shuffled feedback'], order=['H', 'M', 'L', 'Z'])
    sns.violinplot(x = 'coh', y = 'rt', hue = 'model', data = below_thresh_count, inner='points', ax=ax,  palette=['.2', '.5'], hue_order=['Full model', 'Remove feedback'], order=['H', 'M', 'L', 'Z'])
    ...
    ax.legend(handles=[tuple(handles[::3]), tuple(handles[1::3]), tuple(handles[2::3])], labels=below_thresh_count["model"].astype('category').cat.categories.to_list(),
    ...
    plt.savefig(os.path.join(plt_dir, "ablation_rt_below_thresh_count.pdf"), format='pdf')
    plt.savefig(os.path.join(plt_dir, "ablation_rt_below_thresh_count.eps"), format='eps')
    plt.savefig(os.path.join(plt_dir, "ablation_rt_below_thresh_count.png"), format='png')

    df_mean = below_thresh_count[['model', 'coh', 'rt']].groupby(['model', 'coh']).mean()
...
def plot_violin(df):
    ...
    sns.violinplot(x = 'coh', y = 'rt', hue = 'model', data = df, inner='points', ax=ax,  palette=['.2', '.5'], hue_order=['Full model', 'Remove feedback'], order=['H', 'M', 'L', 'Z'])
    ...
    for model in df['model'].unique():
        ...
        oneway_result = f_oneway(temp_df[temp_df['coh']=='H']['rt'], temp_df[temp_df['coh']=='M']['rt'], 
        temp_df[temp_df['coh']=='L']['rt'], temp_df[temp_df['coh']=='Z']['rt'])
        print(oneway_result)
...
def combine_rt_dfs(*args):
    df_list = []
    for df in args:
        temp_df = df[['rep', 'coh', 'rt', 'model']].groupby(['model', 'rep', 'coh']).mean()
        df_list.append(temp_df)
    return pd.concat(df_list).reset_index()
    
def calc_below_thresh_trials(full_df, nofb_df, shuf_df):
    agg_func = {
        'rt': [lambda x: sum(x>500)]
    }
    temp_full_df = full_df[['rep', 'coh', 'rt', 'model']].groupby(['model', 'rep', 'coh']).agg(agg_func)
    temp_nofb_df = nofb_df[['rep', 'coh', 'rt', 'model']].groupby(['model', 'rep', 'coh']).agg(agg_func)
    temp_shuf_df = shuf_df[['rep', 'shuf', 'coh', 'rt', 'model']].groupby(['model', 'rep', 'shuf', 'coh']).agg(agg_func)

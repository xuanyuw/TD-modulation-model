...
def calc_selectivity_val(normalized_h, labels, targ):
    ...
    if targ:
        # reverse the label becasue two groups of neurons receive opposite target color, so after reverse, all differences are green - red
...
def find_corr(
    from_arr, to_arr, mask, w, normalized_h, stim_labels, targ_labels, stim_rng
):
    ...
    if rerun_calculation:
        ...
        for rep in range(total_rep):
            ...
            targ_rng = np.concatenate(
                (range(40, 80), range(170, 180), range(120, 160), range(190, 200)),
                axis=None,
            )
            ...
            stim_rng = np.concatenate(
                (range(0, 40), range(160, 170), range(80, 120), range(180, 190)),
                axis=None,
            )
...
def plot_corr(df, title):
    ...
    fig, ax = plt.subplots()
    sns.stripplot(
        x="conn_type", y="corrcoef", data=df, dodge=True, palette=colors, alpha=0.8
    )
    # plot the mean line
    sns.boxplot(
        showmeans=True,
    ...
    ff_stats = ttest_1samp(
        df["corrcoef"][df["conn_type"] == "Feedforward"].to_numpy(), 0
    )
    fb_stats = ttest_1samp(df["corrcoef"][df["conn_type"] == "Feedback"].to_numpy(), 0)
    total_stats = ttest_1samp(
        df["corrcoef"][df["conn_type"] == "Feedback"].to_numpy(), 0
    )

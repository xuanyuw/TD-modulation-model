...
title = "%s_feedback_weight_comparison"%model_type
if plot_sel:
    title += '_selective'
else:
    title += '_allUnits'
...
def load_data():
    if rerun_calculation:
        ...
        for rep in range(total_rep):
            ...
            # if plot_trained:
            w_rnn_trained =  trained_w['w_rnn0']
            rnn_mask = trained_w['rnn_mask_init']
            # else:
            w_rnn_init = init_w['w_rnn0']
            ...
            #feedback conn trained
            m1_tr2mr_trained = find_weights(m1_red, m1_pref_red, rnn_mask, w_rnn_trained)
            m1_tr2mg_trained = find_weights(m1_red, m1_pref_green, rnn_mask, w_rnn_trained)
            m1_tg2mr_trained = find_weights(m1_green, m1_pref_red, rnn_mask, w_rnn_trained)
            m1_tg2mg_trained = find_weights(m1_green, m1_pref_green, rnn_mask, w_rnn_trained)
            m2_tr2mr_trained = find_weights(m2_red, m2_pref_red, rnn_mask, w_rnn_trained)
            m2_tr2mg_trained = find_weights(m2_red, m2_pref_green, rnn_mask, w_rnn_trained)
            m2_tg2mr_trained = find_weights(m2_green, m2_pref_red, rnn_mask, w_rnn_trained)
            m2_tg2mg_trained = find_weights(m2_green, m2_pref_green, rnn_mask, w_rnn_trained)

            #feedback conn trained
            m1_tr2mr_init = find_weights(m1_red, m1_pref_red, rnn_mask, w_rnn_init)
            m1_tr2mg_init = find_weights(m1_red, m1_pref_green, rnn_mask, w_rnn_init)
            m1_tg2mr_init = find_weights(m1_green, m1_pref_red, rnn_mask, w_rnn_init)
            m1_tg2mg_init = find_weights(m1_green, m1_pref_green, rnn_mask, w_rnn_init)
            m2_tr2mr_init = find_weights(m2_red, m2_pref_red, rnn_mask, w_rnn_init)
            m2_tr2mg_init = find_weights(m2_red, m2_pref_green, rnn_mask, w_rnn_init)
            m2_tg2mr_init = find_weights(m2_green, m2_pref_red, rnn_mask, w_rnn_init)
            m2_tg2mg_init = find_weights(m2_green, m2_pref_green, rnn_mask, w_rnn_init)

            conn = ['mr-tr', 'mg-tr', 'mr-tg', 'mg-tg'] * 4
            conn_type = ['trained'] * 8 + ['init'] * 8
            module = ['m1'] * 4 + ['m2'] * 4 + ['m1'] * 4 + ['m2'] * 4
            rep_num = [rep] * 16
            w_arr = [m1_tr2mr_trained, m1_tr2mg_trained, m1_tg2mr_trained, m1_tg2mg_trained, m2_tr2mr_trained, m2_tr2mg_trained, m2_tg2mr_trained, m2_tg2mg_trained, m1_tr2mr_init, m1_tr2mg_init, m1_tg2mr_init, m1_tg2mg_init, m2_tr2mr_init, m2_tr2mg_init, m2_tg2mr_init, m2_tg2mg_init]
        ...
        df.to_csv(join(f_dir, title+'_fb_conn_data.csv'))
    else:
        df = pd.read_csv(join(f_dir, title+'_fb_conn_data.csv'))
    return df
...
def plot_w_distr(df, rep=None):
    ...
    sns.catplot(x="conn", y="weights",
        hue="conn_type",
        data=df, kind='bar',
        palette="dark", alpha=.6).set(title=title)
